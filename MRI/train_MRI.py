...
def main():
    parser = argparse.ArgumentParser(description='MRI anomaly detection')
    parser.add_argument('--obj', type=str, default='MRI')
    parser.add_argument('--data_type', type=str, default='MRI')
    parser.add_argument('--data_path', type=str, default='D:/chenku/MRI')
    parser.add_argument('--train_data_path', type=str, default='D:/chenku/Mdatasets/MRI/Training/notumor')
    parser.add_argument('--test_normaldata_path', type=str, default='D:/chenku/Mdatasets/MRI/Testing/notumor')
    parser.add_argument('--ab1', type=str, default='D:/chenku/Mdatasets/MRI/Testing/glioma')
    parser.add_argument('--ab2', type=str, default='D:/chenku/Mdatasets/MRI/Testing/meningioma')
    parser.add_argument('--ab3', type=str, default='D:/chenku/Mdatasets/MRI/Testing/pituitary')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        ap = average_precision_score(gt_list, img_scores)
        val_loss = - (img_roc_auc) * 100
        print_log(('epoch: {} image: {:.4f} ap: {:.4f}'.format(epoch, img_roc_auc * 100, ap * 100)), log)
...
def train(proj_layer, decoder, epoch, train_loader, optimizer_proj, optimizer_distill, log, encoder):
    ...
    for (data, img) in tqdm(train_loader):
        ...
        rec_loss = cos(output_noise[1], input[1]) + cos(output_noise[2], input[2]) + cos(output_noise[0], input[0])
