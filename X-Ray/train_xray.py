...
def main():
    parser = argparse.ArgumentParser(description='X-ray anomaly detection')
    parser.add_argument('--obj', type=str, default='X-ray')
    parser.add_argument('--data_type', type=str, default='X-ray')
    parser.add_argument('--train_csv', type=str,
                        default='./Chest X-Ray/siim-acr-pneumothorax/stage_1_train_images.csv')
    parser.add_argument('--test_csv', type=str,
                        default='./Chest X-Ray/siim-acr-pneumothorax/stage_1_test_images.csv')
    parser.add_argument('--train_data_path', type=str, default='./Chest X-Ray/siim-acr-pneumothorax/png_images')
    parser.add_argument('--test_data_path', type=str, default='./Chest X-Ray/siim-acr-pneumothorax/png_images')
    parser.add_argument('--test_mask_path', type=str, default='./Chest X-Ray/siim-acr-pneumothorax/png_masks')
    ...
    train_dataset = PneumoniaDataset(args.train_csv, args.train_data_path)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, **kwargs)

    test_dataset = TestPneumoniaDataset(args.test_csv, args.test_data_path, args.test_mask_path)
...
def train(proj_layer, decoder, epoch, train_loader, optimizer_proj, optimizer_distill, log, encoder):
    ...
    for (data, img, _) in tqdm(train_loader):
